...
source "/opt/gitstatus/gitstatus.prompt.zsh" || return
...
PROMPT='%180F%n%244F@%72F%m%f'                                # user@host
PROMPT+='%252F:%111F%$((-GITSTATUS_PROMPT_LEN-1))<…<%~%<<%f'  # current working directory
PROMPT+='${GITSTATUS_PROMPT:+ $GITSTATUS_PROMPT}'             # git status
PROMPT+=$'\n%240F%* %F{%(?.67.180)}[%j] '                     # new line, time, jobs
PROMPT+='%F{%(?.112.1)}%#%f '                                 # green/red (success/error) %/# (user/root)
